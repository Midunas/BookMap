...
<style scoped>
.scrim { position: fixed; inset: 0; background: rgba(30, 20, 10, .35); z-index: 40; display: flex; justify-content: flex-end; backdrop-filter: blur(2px); }
...
.cov { aspect-ratio: 2/3; border-radius: 6px; overflow: hidden; background: var(--bg-2); border-left: 4px solid var(--c); box-shadow: var(--shadow); }
...
.ratings .v { font-family: var(--serif); font-size: 18px; }
section { margin-top: 22px; }
section h3 { font-size: 15px; color: var(--ink-2); margin-bottom: 8px; font-family: var(--sans); font-weight: 600; }
